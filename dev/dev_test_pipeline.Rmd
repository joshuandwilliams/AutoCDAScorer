...
```{r}
# Generate PCA data object and save to package data
...
run_pca_python <- function(data, n_components = 10) {
  ...
  if (is.list(data)) {
    if (!"images" %in% names(data)) {
      stop("Error: You provided 'data' as a list. It must contain an 'images' element.")
    }
    images <- data$images
    ...
    stop("Error: 'data' must either be numeric (a matrix/array) or a list containing an 'images' element.")
  }
...
# Save predictions
```{r}
annotations_to_csv(new_dataset, new_predictions, "~/Packages/AutoCDAScorer/dev/annotations.csv")
```
