...
# Set up mock CDAScorer file (ignore - only needed for the purposes of this vignette)
...
Your images are suitable if they fall within the cloud of grey datapoints (the images used for model training).
...
```{r}
your_dataset_bgr <- rgb_to_bgr(data = your_dataset)
your_predictions <- predict_score(model = "base_cnn", data = your_dataset_bgr, softmax = FALSE)
...
Optionally you can save these scores for downstream applications.
...
annotations_to_csv(data = your_dataset_bgr, predictions = your_predictions, output_path = predictions_output)
```
